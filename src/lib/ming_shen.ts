...
export function hourBranchFromLocalDate(d: Date): Branch {
  const h = d.getHours();
  // 子: 23-1
  if (h >= 23 || h < 1) return "子";
  const idx = Math.floor((h + 1) / 2); // 1..11
 ...
 * ※本来は流派規則に従って差し替え
 */
export function calcMingPalace(monthGeneral: Branch, hourBranch: Branch): Branch {
 ...
 * 身宮（簡易）：月将と時支で別計算（仮）
 */
export function calcShenPalace_v2(moonPalace: Branch, hourBranch: Branch): Branch {
